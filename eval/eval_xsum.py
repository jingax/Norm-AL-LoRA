...
def evaluate_model(model,dataset,split):
        ...
        for example in dataset[split]:
            inputs = tokenizer(example["document"],padding="max_length",max_length=128,truncation=True,return_tensors="pt")
            ...
            original.append(example["summary"])

        # saving the predictions to tsv file
        with open(f'./outputs/{args.name}/XSUM.tsv', "w", newline="") as f:
